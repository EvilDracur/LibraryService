...
namespace LibraryService.Services.Impl
{
    public class LibraryRepository : ILibraryRepositoryService
    {
        ...
        public IList<string> GetDescriptionByTitle(string title) => GetByTitle(title).Select(book => book.ToString()).ToList();

        public IList<string> GetDescriptionByAuthor(string authorName) => GetByAuthor(authorName).Select(book => book.ToString()).ToList();

        public IList<string> GetDescriptionByCategory(string category) => GetByCategory(category).Select(book => book.ToString()).ToList();
